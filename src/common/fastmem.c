...
void init_memory (void)
{
	...
	//check if it's a zblorb file
	if(strncmp("FORM",zmp,4) == 0){
		fseek(story_fp,0,SEEK_SET);
		
		bool found = 0;
		char c = fgetc(story_fp);
			...
			if(c == 'Z'){
				...
				if(c == 'C'){
					...
					if(c == 'O'){
						...
						if(c == 'D'){
							printf("Found ZCOD header\n");
							
							//skip over header junk
							fgetc(story_fp);
							fgetc(story_fp);
							fgetc(story_fp);
							fgetc(story_fp);
							
							exe_start = ftell(story_fp);
							fread(zmp,1,64,story_fp);
							found = 1;
						}
					}
				}
    ...
    if (h_version <= V4) {
        ...
        op1_opcodes[0x0f] = z_not-p_start;
    } else {
        op0_opcodes[0x09] = z_catch-p_start;
        op1_opcodes[0x0f] = z_call_n-p_start;
    }
...
void z_restart (void)
{
    ...
    if (!first_restart) {
        ...
        fseek (story_fp, exe_start, SEEK_SET);
...
void z_restore (void)
{
    ...
    if (zargc != 0) {
        ...
        if (f_setup.save_quetzal) {
			...
			exe_start = fgetc(gfp);
			exe_start = (exe_start << 8)|fgetc(gfp);
			exe_start = (exe_start << 8)|fgetc(gfp);
			exe_start = (exe_start << 8)|fgetc(gfp);
...
void z_save (void)
{
    ...
    if (zargc != 0) {
        ...
        if (f_setup.save_quetzal) {
            ...
            /* Write game file */
			fputc(exe_start >> 24,gfp);
			fputc(exe_start >> 16,gfp);
			fputc(exe_start >> 8,gfp);
			fputc(exe_start,gfp);
