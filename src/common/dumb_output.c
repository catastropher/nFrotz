...
void clear_status_line(){
	int i;
	for(i = 0; i < 54;i++){
		status_cell[i] = make_cell(current_style, ' ');
	}
}
...
{
	if(cwin == 1){
		status_cell[status_cell_pos++] = make_cell(current_style, c);
		
		printf("status = ");
		
		int i;
		for(i = 0;i < status_cell_pos;i++){
			printf("%c",status_cell[i]);
		}
...
static void show_cell(cell cel)
{
    ...
    switch (cell_style(cel)) {
        case 0:
            ...
            break;
        case PICTURE_STYLE:
            DISP("%c", show_pictures ? c : ' ');
            break;
        case REVERSE_STYLE:
            if (c == ' ')
                DISP("%c", rv_blank_char);
            else
                switch (rv_mode) {
                    case RV_NONE: DISP("%c", c); break;
                    case RV_CAPS: DISP("%c", toupper(c)); break;
                    case RV_UNDERLINE: DISP("%c", '_'); DISP("%c", '\b'); DISP("%c", c); break;
                    case RV_DOUBLESTRIKE: DISP("%c", c); DISP("%c", '\b'); DISP("%c", c); break;
                }
            break;
    }
}
...
{
	if(cwin == 1){
		printf("disp disp, ssp = %d\n",status_cell_pos);
		nio_clear(&status_line);
		int i;
		for(i = 0;i < 54;i++){
			show_cell(status_cell[i]);
			status_cell[i] = make_cell(current_style,' ');
		}
		status_cell_pos = 0;
		return;
